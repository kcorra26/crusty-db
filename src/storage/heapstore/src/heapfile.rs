...
pub(crate) struct HeapFile {
    ...
    pub pathbuf: PathBuf, // NOTE: REMOVE THIS
...
impl HeapFile {
    ...
    pub(crate) fn new(file_path: PathBuf, container_id: ContainerId) -> Result<Self, CrustyError> {
        ...
        Ok(HeapFile {
            ...
            pathbuf: file_path, // NOTE THIS IS EXTREMELY SLOW
            container_id,
    ...
    pub fn num_pages(&self) -> PageId {
        (self.pathbuf.metadata().unwrap().len() as usize / PAGE_SIZE) as PageId
    }
    ...
    pub(crate) fn read_page_from_file(&self, pid: PageId) -> Result<Page, CrustyError> {
        ...
        let start_loc: u64 = pid as u64 * PAGE_SIZE as u64;
        let end_loc = start_loc + PAGE_SIZE as u64;

        let mut file: std::sync::RwLockWriteGuard<'_, File> = self.file_lock.write().unwrap();
    ...
    pub(crate) fn write_page_to_file(&self, page: &Page) -> Result<(), CrustyError> {
        ...
        let loc = page.get_page_id() as u64 * PAGE_SIZE as u64;

        let mut file: std::sync::RwLockWriteGuard<'_, File> = self.file_lock.write().unwrap();
        file.seek(SeekFrom::Start(loc))?;
        let bytes_amt = file.write(bytes)?;
