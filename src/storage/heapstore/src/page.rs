...
impl Page {
    ...
    pub fn new(page_id: PageId) -> Self {
        ...
        let mut new_array: [u8; PAGE_SIZE] = [0; PAGE_SIZE];
        let pageid_size: usize = std::mem::size_of::<PageId>(); // size of PageId type
        new_array[..pageid_size].copy_from_slice(&id_bytes);
        Self { data: new_array }
    }
    ...
    pub fn get_page_id(&self) -> PageId {
        let pgid_size = std::mem::size_of::<PageId>();
        PageId::from_le_bytes(self.data[..pgid_size].try_into().unwrap())
    }
